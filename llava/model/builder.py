...
def load_lora_model(lora_path, model_base, base_model_name, is_ckpt=False, load_8bit=False, load_4bit=False, device_map="auto", attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    tokenizer, model, image_processor, context_len = load_pretrained_model(model_base,None,base_model_name,load_8bit,load_4bit,device_map,attn_implementation,customized_config,overwrite_config,**kwargs)
    if lora_path is None:
        lora_path = []
    if type(lora_path) == str:
        lora_path = [lora_path,]
    for lora in lora_path:
        rank0_print(f"Loading LoRA: {lora}")
        rank0_print(f"Loading additional LLaVA weights...")
        if "checkpoint-" in lora:
            ...
            non_lora_trainables = torch.load(os.path.join(lora,f"global_step{num}/mp_rank_00_model_states.pt"), map_location=next(model.parameters()).device)["module"]

        else:
            if os.path.exists(os.path.join(lora, "non_lora_trainables.bin")):
                non_lora_trainables = torch.load(os.path.join(lora, "non_lora_trainables.bin"), map_location=next(model.parameters()).device)
            else:
                # this is probably from HF Hub
                from huggingface_hub import hf_hub_download
